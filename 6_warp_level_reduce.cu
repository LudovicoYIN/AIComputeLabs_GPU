...
/**
 * 总体思路：把计算都放到Warp中，其中：
 * Intra-warp Data Sharing (warp内数据共享）的好处
 * 1. Divergence-free Programming（无分歧编程）
 *      在warp内进行数据共享时，由于warp内的所有线程同步执行相同的指令，没有线程分歧（divergence）问题。这意味着所有线程始终在同一步骤执行相同的操作，避免了warp分裂带来的性能损失。
 * 2. “Free” Synchronization among Threads（线程间的“免费”同步）
 *      Warp内的线程天然同步，因为同一个warp内的所有线程总是同步执行。因此，不需要显式的同步指令（如__syncthreads()）来协调warp内的线程。这简化了编程并减少了同步开销。
 * Compared to Shared Memory（与共享内存相比）
 * 1. No __syncthreads Overhead（没有__syncthreads的开销）
 *      在使用共享内存进行线程间通信时，需要使用__syncthreads()来确保所有线程在进行下一步之前都完成了当前步骤。__syncthreads()有一定的开销，尤其是在大量使用时，会影响性能。而warp内操作不需要显式同步，避免了这一开销。
 * 2. Significantly Higher Communication Bandwidth（显著更高的通信带宽）
 *      Warp shuffle指令允许warp内的线程直接通过寄存器文件进行数据交换，这比通过共享内存进行通信要快得多。共享内存通信需要存取共享内存，而warp shuffle指令则利用更快的寄存器文件进行数据交换，因此具有更高的通信带宽。
 * @tparam blockSize
 * @param d_in
 * @param d_out
 * time latency: 0.32ms
